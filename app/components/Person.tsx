...
export default function Person({ name, title, description, image, imagePosition }: PersonProps) {
    ...
    return (
        <div className="flex flex-col md:flex-row items-center gap-6 sm:gap-8 md:gap-12 w-full bg-white p-20 rounded-4xl">
            {/* Image - Mobile: appears first, Desktop: position based on imagePosition */}
            <div className={`flex items-center justify-center w-full md:w-auto shrink-0 order-1 ${isImageLeft ? "md:order-1" : "md:order-2"}`}>
            ...
            <div className={`flex flex-col gap-4 sm:gap-6 w-full md:flex-1 order-2 ${isImageLeft ? "md:order-2" : "md:order-1"}`}>
