...
export default function SolutionCard({
  ...
  const isNursingLeadIcon = icon.toLowerCase().includes("nursinglead");

  return (
    <div
      className="bg-[#3863a3] flex flex-col gap-4 items-center justify-center px-4 sm:px-6 md:px-6 py-8 sm:py-10 md:py-10 rounded-[32px] flex-1"
      ...
      <div className="bg-white flex items-center justify-center p-3 rounded-full w-[100px] h-[100px] shrink-0">
        <div className="relative w-[80px] h-[80px]">
          <img
            src={icon}
            alt={iconAlt}
            className={`w-full h-full object-contain ${isNursingLeadIcon ? "p-2" : ""}`}
          />
        </div>
      </div>
